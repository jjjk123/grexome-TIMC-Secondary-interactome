...
def calculate_harmonic_centrality(interactome, causal_genes) -> dict:
    '''
    Calculates harmonic centrality for every gene in the interactome.
    ...
    returns:
    - scores: dict with key=gene, value=harmonic_centrality
    '''
    # get non-causal genes
    nbunch = [k for k, v in causal_genes.items() if v == 0]
    # get causal genes
    sources = [k for k, v in causal_genes.items() if v == 1]

    scores = networkx.harmonic_centrality(interactome, nbunch=nbunch, sources=sources)
...
def scores_to_TSV(scores, out_path):
    ...
    '''
    out_file = out_path / "harmonic_centrality.tsv"
    f = open(out_file, 'w+')
...
def main(interactome_file, causal_genes_file, canonical_genes_file, out_path):
    ...
    harmonic_centrality = calculate_harmonic_centrality(interactome, causal_genes)

    logger.info("Done!")
    scores_to_TSV(harmonic_centrality, out_path)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Calculate harmonic centrality for new candidates of infertility based on the guilt-by-association approach."
    )
